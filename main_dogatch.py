...
def save_content(url, content):
    ...
    if not path or 'detail' not in path:
        return None

    if path[-6:] != 'detail':
        path_list = path.split('/')
        path = '{}_{}'.format('/'.join(path_list[:-1]), path_list[-1])
...
def request_href(url):
    ...
    for href in href_list:
        ...
        if href:
            if 'photo/0/' in href:
                href = href.replace('photo/0', 'detail')
            if 'intro' in href:
                href = href.replace('intro', 'detail')
